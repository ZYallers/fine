...
import (
	...
	"github.com/ZYallers/fine/util/frand"
...
func (a *Array) Set(index int, value interface{}) error {
	...
	defer a.mu.Unlock()
	if index < 0 || index >= len(a.array) {
		return errors.Errorf("index %d out of array range %d", index, len(a.array))
	}
	a.array[index] = value
	return nil
}
...
func (a *Array) PopRand() (value interface{}, found bool) {
	a.mu.Lock()
	defer a.mu.Unlock()
	return a.doRemoveWithoutLock(frand.Intn(len(a.array)))
}
...
func (a *Array) PopRands(size int) []interface{} {
	...
	for i := 0; i < size; i++ {
		array[i], _ = a.doRemoveWithoutLock(frand.Intn(len(a.array)))
	}
	return array
}
...
func (a *Array) Merge(array interface{}) *Array {
	return a.Append(fconv.Interfaces(array)...)
}
...
func (a *Array) Fill(startIndex int, num int, value interface{}) error {
	...
	if startIndex < 0 || startIndex > len(a.array) {
		return errors.Errorf("index %d out of array range %d", startIndex, len(a.array))
	}
...
func (a *Array) Rand() (value interface{}, found bool) {
	...
	return a.array[frand.Intn(len(a.array))], true
}
...
func (a *Array) Rands(size int) []interface{} {
	...
	array := make([]interface{}, size)
	for i := 0; i < size; i++ {
		array[i] = a.array[frand.Intn(len(a.array))]
	}
	return array
}
...
func (a *Array) Shuffle() *Array {
	a.mu.Lock()
	defer a.mu.Unlock()
	for i, v := range frand.Perm(len(a.array)) {
...
func (a *Array) Join(glue string) string {
	...
	for k, v := range a.array {
		buffer.WriteString(fconv.String(v))
		if k != len(a.array)-1 {
...
func (a *Array) String() string {
	...
	for k, v := range a.array {
		...
		if fstr.IsNumeric(s) {
			...
			buffer.WriteString(`"` + fstr.QuoteMeta(s, `"\`) + `"`)
		}
		if k != len(a.array)-1 {
			buffer.WriteByte(',')
		}
...
func (a *Array) UnmarshalValue(value interface{}) error {
	...
	switch value.(type) {
	case string, []byte:
		return json.UnmarshalUseNumber(fconv.Bytes(value), &a.array)
	default:
		a.array = fconv.SliceAny(value)
	}
	return nil
}
