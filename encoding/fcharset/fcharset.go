// Package fcharset implements character-set conversion functionality.
//
...
func Convert(dstCharset string, srcCharset string, src string) (dst string, err error) {
	...
	if srcCharset != "UTF-8" {
		if e := getEncoding(srcCharset); e != nil {
			...
			return dst, errors.Errorf(`unsupported srcCharset "%s"`, srcCharset)
		}
	}
	...
	if dstCharset != "UTF-8" {
		if e := getEncoding(dstCharset); e != nil {
			...
			return dst, errors.Errorf(`unsupported dstCharset "%s"`, dstCharset)
		}
	} else {
		dst = src
	}
