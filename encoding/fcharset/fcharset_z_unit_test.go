...
func TestDecode(t *testing.T) {
	for _, data := range testData {
		str := ""
		str, err := fcharset.Convert("UTF-8", data.otherEncoding, data.other)
		if err != nil {
...
func TestUTF8To(t *testing.T) {
	for _, data := range testData {
		str := ""
		str, err := fcharset.UTF8To(data.otherEncoding, data.utf8)
		if err != nil {
			t.Errorf("Could not create decoder for %v", err)
			continue
		}
...
func TestToUTF8(t *testing.T) {
	for _, data := range testData {
		str := ""
		str, err := fcharset.ToUTF8(data.otherEncoding, data.other)
		if err != nil {
			t.Errorf("Could not create decoder for %v", err)
			continue
		}
...
func TestEncode(t *testing.T) {
	for _, data := range testData {
		str := ""
		str, err := fcharset.Convert(data.otherEncoding, "UTF-8", data.utf8)
		if err != nil {
			t.Errorf("Could not create decoder for %v", err)
			continue
		}
...
func TestConvert(t *testing.T) {
	...
	str, err := fcharset.Convert(dstCharset, srcCharset, src)
	if err != nil {
...
func TestConvertErr(t *testing.T) {
	ftest.C(t, func(t *ftest.T) {
		...
		s1, e1 := fcharset.Convert(srcCharset, srcCharset, src)
		t.Assert(e1, nil)
		t.Assert(s1, src)

		s2, e2 := fcharset.Convert(dstCharset, "no this charset", src)
		t.AssertNE(e2, nil)
		t.Assert(s2, src)

		s3, e3 := fcharset.Convert("no this charset", srcCharset, src)
		t.AssertNE(e3, nil)
		t.Assert(s3, src)
	})
}
...
func TestSupported(t *testing.T) {
	ftest.C(t, func(t *ftest.T) {
		t.Assert(fcharset.Supported("UTF-8"), true)
		t.Assert(fcharset.Supported("UTF-80"), false)
	})
}
