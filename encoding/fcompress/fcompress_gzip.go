...
func Gzip(data []byte, level ...int) ([]byte, error) {
	...
	if len(level) > 0 {
		...
		if err != nil {
			err = errors.Wrapf(err, `gzip.NewWriterLevel failed for level "%d"`, level[0])
			return nil, err
		}
	} else {
	...
	if _, err = writer.Write(data); err != nil {
		err = errors.Wrap(err, `writer.Write failed`)
		return nil, err
	}
	if err = writer.Close(); err != nil {
		err = errors.Wrap(err, `writer.Close failed`)
		return nil, err
	}
...
func GzipFile(srcFilePath, dstFilePath string, level ...int) (err error) {
	dstFile, err := ffile.Create(dstFilePath)
	if err != nil {
		return err
	}
	defer dstFile.Close()
...
func GzipPathWriter(filePath string, writer io.Writer, level ...int) error {
	...
	srcFile, err := ffile.Open(filePath)
	if err != nil {
		return err
	}
	defer srcFile.Close()
	...
	if len(level) > 0 {
		...
		if err != nil {
			return errors.Wrap(err, `gzip.NewWriterLevel failed`)
		}
	} else {
...
func UnGzip(data []byte) ([]byte, error) {
	...
	if err = reader.Close(); err != nil {
		err = errors.Wrap(err, `reader.Close failed`)
...
func UnGzipFile(srcFilePath, dstFilePath string) error {
	srcFile, err := ffile.Open(srcFilePath)
	if err != nil {
		return err
	}
	defer srcFile.Close()
