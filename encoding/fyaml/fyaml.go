// Package fyaml provides accessing and converting for YAML content.
package fyaml
...
func Encode(value interface{}) (out []byte, err error) {
	if out, err = yaml.Marshal(value); err != nil {
		err = errors.Wrap(err, `yaml.Marshal failed`)
	}
	return
}
...
func Decode(content []byte) (map[string]interface{}, error) {
	...
	if err = yaml.Unmarshal(content, &result); err != nil {
		err = errors.Wrap(err, `yaml.Unmarshal failed`)
		return nil, err
	}
	return fconv.MapDeep(result), nil
}
