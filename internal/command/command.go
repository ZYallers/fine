		...
		if len(array) > 2 {
			if array[2] == "=" {
				...
				if len(args[i+1]) > 0 && args[i+1][0] == '-' {
					// Example: fine gen -d -n 1
					parsedOptions[array[1]] = array[3]
				} else {
					// Example: fine gen -n 2
...
//
// Fetching Rules:
// 1. Command line arguments are in lowercase format, eg: fine.package.variable;
// 2. Environment arguments are in uppercase format, eg: FINE_PACKAGE_VARIABLE；
