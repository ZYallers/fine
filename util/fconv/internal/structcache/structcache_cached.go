...
type CommonConverter struct {
	...
	FTime   func(any interface{}, format ...string) *ftime.Time
...
func GetCachedStructInfo(structType reflect.Type, priorityTag string) *CachedStructInfo {
	...
	if priorityTag != "" {
		priorityTagArray = append(utils.SplitAndTrim(priorityTag, ","), ftag.StructTagPriority...)
	} else {
		priorityTagArray = ftag.StructTagPriority
	}
