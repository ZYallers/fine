// Package fuid provides simple and high performance unique id generation functionality.
package fuid
...
var (
	sequence     ftype.Uint32 // Sequence for unique purpose of the current process.
...
func init() {
	...
	if len(macs) > 0 {
		...
		s := strconv.FormatUint(uint64(fhash.SDBM(macAddrBytes)), 36)
		copy(b, s)
		macAddrStr = string(b)
	}
...
func S(data ...[]byte) string {
	...
	if len(data) == 0 {
		...
		panic(errors.New("too many data parts, it should be no more than 2 parts"))
	}
	return string(b)
}
...
func getDataHashStr(data []byte) []byte {
	b := []byte{'0', '0', '0', '0', '0', '0', '0'}
	s := strconv.FormatUint(uint64(fhash.SDBM(data)), 36)
	copy(b, s)
	return b
}
