...
const (
	pathFilterKey = "/test/ftest/ftest"
)
...
func C(t *testing.T, f func(t *T)) {
	defer func() {
		if err := recover(); err != nil {
			_, _ = fmt.Fprintf(os.Stderr, "%v\n%s", err, fdebug.StackWithFilter([]string{pathFilterKey}))
			t.Fail()
		}
	}()
	f(&T{t})
...
func Assert(value, expect interface{}) {
	...
	var (
		strValue  = fconv.String(value)
		strExpect = fconv.String(expect)
	)
	...
	if t1 == t2 {
		panic(
			fmt.Sprintf(
				...
				fconv.String(value), t1, fconv.String(expect), t2,
			),
		)
	}
	// Value assert.
...
func AssertGT(value, expect interface{}) {
	...
	case reflect.String:
		passed = fconv.String(value) > fconv.String(expect)

	case reflect.Int, reflect.Int8, reflect.Int16, reflect.Int32, reflect.Int64:
		passed = fconv.Int(value) > fconv.Int(expect)

	case reflect.Uint, reflect.Uint8, reflect.Uint16, reflect.Uint32, reflect.Uint64:
		passed = fconv.Uint(value) > fconv.Uint(expect)

	case reflect.Float32, reflect.Float64:
		passed = fconv.Float64(value) > fconv.Float64(expect)
	}
	if !passed {
		panic(fmt.Sprintf(`[ASSERT] EXPECT %v > %v`, value, expect))
	}
...
func AssertGE(value, expect interface{}) {
	...
	case reflect.Int, reflect.Int8, reflect.Int16, reflect.Int32, reflect.Int64:
		passed = fconv.Int64(value) >= fconv.Int64(expect)

	case reflect.Uint, reflect.Uint8, reflect.Uint16, reflect.Uint32, reflect.Uint64:
		passed = fconv.Uint64(value) >= fconv.Uint64(expect)
...
func AssertIN(value, expect interface{}) {
	...
	case reflect.Slice, reflect.Array:
		expectSlice := fconv.Strings(expect)
		for _, v1 := range fconv.Strings(value) {
			result := false
...
func AssertNI(value, expect interface{}) {
	...
	case reflect.Slice, reflect.Array:
		for _, v1 := range fconv.Strings(value) {
			result := true
			for _, v2 := range fconv.Strings(expect) {
				if v1 == v2 {
					result = false
					break
				}
...
func Fatal(message ...interface{}) {
	_, _ = fmt.Fprintf(
		...
		fdebug.StackWithFilter([]string{pathFilterKey}),
	)
	os.Exit(1)
}
...
func compareMap(value, expect interface{}) error {
	...
	for _, key := range ksValue {
		mValue[fconv.String(key.Interface())] = fconv.String(rvValue.MapIndex(key).Interface())
	}

	for _, key := range ksExpect {
		mExpect[fconv.String(key.Interface())] = fconv.String(rvExpect.MapIndex(key).Interface())
	}
...
func DataPath(names ...string) string {
	_, path, _ := fdebug.CallerWithFilter([]string{pathFilterKey})
