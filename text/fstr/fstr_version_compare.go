...
func VersionCompare(version1, version2, operator string) bool {
	compare := doVersionCompare(version1, version2)
	switch operator {
	case "<", "lt":
		return compare == -1
	case "<=", "le":
	...
	case "!=", "<>", "ne":
		return compare != 0
	default:
		panic("version compare operator: invalid")
	}
}

func doVersionCompare(origV1, origV2 string) int {
	if origV1 == "" || origV2 == "" {
		if origV1 == "" && origV2 == "" {
			return 0
		}
	...
	if origV1[0] == '#' {
		...
		ver1 = canonical(origV1)
	}
	if origV2[0] == '#' {
		ver2 = origV2
	} else {
		ver2 = canonical(origV2)
	}
	n1, n2 := 0, 0
	for {
		p1, p2 := "", ""
	...
	if compare == 0 {
		if ver1 != "" {
			if ver1[0] >= '0' && ver1[0] <= '9' {
				...
				compare = doVersionCompare(ver1, "#N#")
			}
		} else if ver2 != "" {
			if ver2[0] >= '0' && ver2[0] <= '9' {
				compare = -1
			} else {
				compare = doVersionCompare("#N#", ver2)
			}
		}
	}
