...
func NewRequest(url string) *Request {
	r := &Request{Url: url, client: HttpClient, Timeout: ClientTimeout}
...
func (r *Request) SetPostData(data map[string]interface{}) *Request {
	...
	if ct, ok := r.Headers["Content-Type"]; ok && strings.HasPrefix(ct, "application/json") {
		if pb, err := fjson.Marshal(r.PostData); err != nil {
			r.error = err
		} else {
			r.body = string(pb)
		}
