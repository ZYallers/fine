...
func Copy(src string, dst string, option ...CopyOption) error {
	...
	if srcStatErr != nil {
		...
		return errors.Wrapf(
			srcStatErr, `call os.Stat on "%s" failed`, src,
		)
	}
	dstStat, dstStatErr := os.Stat(dst)
	if dstStatErr != nil && !os.IsNotExist(dstStatErr) {
		return errors.Wrapf(dstStatErr, `call os.Stat on "%s" failed`, dst)
	}
...
func CopyFile(src, dst string, option ...CopyOption) (err error) {
	...
	if srcStatErr != nil {
		...
		return errors.Wrapf(srcStatErr, `call os.Stat on "%s" failed`, src)
	}
	dstStat, dstStatErr := os.Stat(dst)
	if dstStatErr != nil && !os.IsNotExist(dstStatErr) {
		return errors.Wrapf(dstStatErr, `call os.Stat on "%s" failed`, dst)
