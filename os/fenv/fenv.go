// Package fenv provides operations for environment variables of system.
package fenv
...
// in the environment.
func Get(key string, def ...interface{}) *fvar.Var {
	v, ok := os.LookupEnv(key)
	if !ok {
		if len(def) > 0 {
			return fvar.New(def[0])
		}
		return nil
	}
	return fvar.New(v)
...
//
// Fetching Rules:
// 1. Environment arguments are in uppercase format, eg: FINE_<package name>_<variable name>；
// 2. Command line arguments are in lowercase format, eg: fine.<package name>.<variable name>;
func GetWithCmd(key string, def ...interface{}) *fvar.Var {
