...
func ConvertZone(strTime string, toZone string, fromZone ...string) (*Time, error) {
	...
	if l, err = time.LoadLocation(toZone); err != nil {
		err = errors.Wrapf(err, `time.LoadLocation failed for name "%s"`, toZone)
		return nil, err
	} else {
...
func StrToTimeLayout(str string, layout string) (*Time, error) {
	if t, err := time.ParseInLocation(layout, str, time.Local); err == nil {
		...
		return nil, errors.Wrapf(err, `time.ParseInLocation failed for layout "%s" and value "%s"`, layout, str)
	}
}
...
func ParseTimeFromContent(content string, format ...string) *Time {
	...
	if len(format) > 0 {
		for _, item := range format {
			match, _ = fregex.MatchString(formatToRegexPattern(item), content)
			if len(match) > 0 {
...
func ParseDuration(s string) (duration time.Duration, err error) {
	...
	match, err := fregex.MatchString(`^([\-\d]+)[dD](.*)$`, s)
	if err != nil {
		return 0, err
	}
	if len(match) == 3 {
